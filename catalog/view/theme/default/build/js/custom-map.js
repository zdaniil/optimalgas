...
function initCustomMap() {
	...
	var centerCoord = new google.maps.LatLng(50.007768, 36.278790);
...
function CustomMap(opt) {
	this.options = jQuery.extend({
		holder: null,
		map: '.map-canvas',
		startCooords: [50.007768, 36.278790],
		defaultOptions: {
			maxZoom: 16,
			zoom: 5
		}
...
CustomMap.prototype = {
	...
	findElements: function() {
		...
		this.markers = [];
		this.skrollMap = this.options.defaultOptions.scrollwheel;
	},
	createMap: function() {
		var self = this,
		usRoadMapType;
		...
		google.maps.event.addListener(this.mapCanvas, 'click', function(event){
			self.skrollMap = !self.skrollMap;
			this.setOptions({scrollwheel: self.skrollMap});
		});
	},
	...
	addMarker: function(obj) {
		...
		function openInfoPopup(marker, infoBox) {
			...
			var content = infoBox.getContent(self.mapCanvas, marker);
			var newContent = jQuery('<div>', {html: content});

			newContent.find('.popup-holder').addClass(activeClass);
			infoBox.setContent(newContent.html())
		...
		function closeInfoPopup(marker, infoBox) {
			...
			newContent.find('.popup-holder').removeClass(activeClass);
			infoBox.setContent(newContent.html())
